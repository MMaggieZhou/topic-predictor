...
def calculate(knn, x, feature_num,the_class_frequency,similarity_fn,feature_weights):
    ...
    if similarity_fn:
        ...
        for i in range(len(knn.xs)):#num_article
            ...
            for j in feature_num:
                ...
                similarity+= (1- math.fabs(float(x[int(j)]) - float(knn.xs[i][int(j)])))*float(feature_weights[int(j)])
            order.append((similarity, i))
    order.sort()#similarity,article_num

    # first 'k' are the ones I want.
    weights = {}  # class -> number of votes
    for similarity, i in order[:knn.k]:
        binary_label = knn.ys[i]
        if binary_label not in weights:
            weights[binary_label] =0
        else:
            if int(binary_label) == 0:
                weights[binary_label] += (similarity*float(the_class_frequency))
            else :
                weights[binary_label]+= similarity*(1-float(the_class_frequency)) 
    return weights

def classify(knn, x, feature_num,the_class_frequency,similarity_fn,feature_weights):
    ...
    """
    weights = calculate(
        knn, x,  feature_num,the_class_frequency,similarity_fn,feature_weights)
    
    if '1' not in weights:
        return 0
    elif '0' not in weights :
        return 1
